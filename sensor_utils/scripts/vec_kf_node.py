...
class EKFLocalizer(Node):
    ...
    def gnss_callback(self, msg: Odometry):
        ...
        # First GNSS measurement or after reset
        ...
        # Only check if robot has moved enough
        if norm_pred > 0.1:
            ...
            if angle_deg > self.VEC_DIFF_DEG_THRESHOLD:
                self.gnss_resume_time = curr_stamp + self.WAIT_SEC_VEC_DIFF
                #self.get_logger().info(f"GNSS update skipped due to large heading discrepancy: {angle_deg:.2f} deg")
                return
