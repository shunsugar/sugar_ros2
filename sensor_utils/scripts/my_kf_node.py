...
class EKFLocalizer(Node):
    ...
    def __init__(self):
        ...
        self.debug_factor_pub = self.create_publisher(
            Float32, "/ekf/debug/inflation_factor", 10
        )
    ...
    def gnss_callback(self, msg: Odometry):
        ...
        if norm_pred > 0.2:  # 移動量が小さいときは方向計算が不安定なのでスキップ
            ...
            if angle_deg > 15:
                angle_deg = 15
            instant_inflation = 1.0 + (angle_deg**2) * 1.5
