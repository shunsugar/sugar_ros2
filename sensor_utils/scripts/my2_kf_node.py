...
class EKFLocalizer(Node):
    ...
    def __init__(self):
        ...
        # self.debug_factor_pub = self.create_publisher(Float32, "/ekf/debug/inflation_factor", 10)
        ...
        self.avg_heading_error_deg = 0.0
        self.HEADING_BIAS_ALPHA = 0.05
    ...
    def gnss_callback(self, msg: Odometry):
        ...
        if norm_pred > 0.2:  # 移動量が小さいときは方向計算が不安定なのでスキップ
            # 1. 外積(cross)と内積(dot)を計算
            cross = dp_gnss_vec[0] * dp_pred_vec[1] - dp_gnss_vec[1] * dp_pred_vec[0]
            dot = dp_gnss_vec[0] * dp_pred_vec[0] + dp_gnss_vec[1] * dp_pred_vec[1]

            # 2. 符号付きの角度差を計算 (-180 ~ +180度)
            # abs() を外して、左ズレか右ズレかを区別します
            raw_angle_rad = math.atan2(cross, dot)
            raw_angle_deg = math.degrees(raw_angle_rad)

            # 3. バイアス（ドリフト成分）の更新
            # ローパスフィルタで「普段のズレ」を学習する
            # マルチパスのような「突発的な大ズレ」を学習しすぎないよう、
            # 角度差が大きすぎる時は学習を止めるor弱めるガードを入れるのがコツです
            if abs(raw_angle_deg) < 20.0:  # 20度以内のズレならドリフトとして徐々に学習
                self.avg_heading_error_deg = (
                    (1.0 - self.HEADING_BIAS_ALPHA) * self.avg_heading_error_deg
                    + self.HEADING_BIAS_ALPHA * raw_angle_deg
                )

            # 4. バイアスを除去した「純粋な乖離（マルチパス成分）」を抽出
            # これが「右肩上がり」にならない値になります
            dynamic_angle_deg = abs(raw_angle_deg - self.avg_heading_error_deg)

            # デバッグログ（必要に応じて有効化）
            # self.get_logger().info(f"Raw: {raw_angle_deg:.1f}, Bias: {self.avg_heading_error_deg:.1f}, Diff: {dynamic_angle_deg:.1f}")

            # 5. ペナルティ計算（dynamic_angle_deg を使用）
            if dynamic_angle_deg > 15:
                dynamic_angle_deg = 15

            # ここで判定に使います
            instant_inflation = 1.0 + (dynamic_angle_deg**2) * 2.0
        ...
        if self.current_gnss_covariance_scale > INFLATION_CUTOFF_THRESHOLD:
            self.get_logger().warn(
                f"Updates skipped due to high uncertainty: factor {self.current_gnss_covariance_scale:.1f}"
            )
            self.prev_gnss_stamp = curr_stamp
            self.prev_gnss_pos = pos
            return  # ここで帰る
        ...
        # debug_msg = Float32()
        # debug_msg.data = float(self.current_gnss_covariance_scale)
        # self.debug_factor_pub.publish(debug_msg)

        if self.current_gnss_covariance_scale > 2.0:
            self.get_logger().info(
                f"R scaled by x{self.current_gnss_covariance_scale:.2f}"
            )
